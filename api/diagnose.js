...
function buildCorpusForPrompt() {
  return Object.entries(CATEGORY_LIBRARY).map(([category, posts]) => ({
    category,
    hot_posts: posts.map((post) => ({
      title: post.title,
...
async function runClaudeDiagnosis(userNote) {
  const systemPrompt =
    "你是内容诊断专家。你将收到用户笔记和多品类爆款语料库。你需要先判断最相似品类，再从标题结构、开头钩子、信息密度、互动引导四个维度做评分与建议。必须只输出 JSON，不要输出任何解释文字或 Markdown。";

  const userPrompt = {
    task: "请先从全部品类中判断最相似品类，再做结构化诊断",
    dimensions: ["标题结构", "开头钩子", "信息密度", "互动引导词"],
    user_note: userNote,
    category_corpus: buildCorpusForPrompt(),
    output_rules: [
      "仅输出合法 JSON",
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const diagnosis = await runClaudeDiagnosis({ title, content, tags });
